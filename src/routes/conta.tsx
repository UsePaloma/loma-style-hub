  ...
  return (
    <div className="mx-auto max-w-md">
      <div className="text-center">
        ...
        <h1 className="mt-2 font-display text-4xl">Minha conta</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          Entre para acompanhar seus pedidos e ajustar entrega, endereço e tamanho.
        </p>
      </div>
      ...
      {tab === "login" ? (
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            const res = loginCustomer(login.email, login.password);
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <Button variant="outline" className="rounded-full" onClick={logoutCustomer}>
      ...
      <div className="space-y-4">
        ...
        {myOrders.length === 0 ? (
          <p className="rounded-2xl bg-card p-6 text-sm text-muted-foreground shadow-soft">
            Você ainda não tem pedidos por aqui. Ao fazer um pedido pelo site logada, ele aparece
            nesta lista.
          </p>
        ) : (
    ...
    <form
      ...
      onSubmit={(e) => {
        ...
        toast[ok ? "success" : "error"](
          ok ? "Pedido atualizado ♡" : "Não foi possível atualizar este pedido.",
        );
      }}
    >
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Pedido #{order.id} · {order.type} · {brl(order.total)}
          </p>
        </div>
      ...
      <div className="space-y-1.5">
        <Label className="flex items-center gap-2">
          <Ruler className="h-4 w-4" /> Tamanho da peça
        </Label>
