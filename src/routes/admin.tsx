...
function CustomersTab() {
  ...
  async function saveCustomer(customer: Customer) {
    if (!supabase) return toast.error("Supabase não está configurado.");
    const { data: row, error } = await supabase.from("site_data").select("data").eq("id", 1).single();
    if (error) return toast.error("Não foi possível carregar os dados.");
    ...
    if (result.error) return toast.error("Falha ao salvar cliente.");
  ...
  async function removeCustomer(id: string) {
    ...
    const result = await supabase.from("site_data").upsert({ id: 1, data: { ...data, customers: next }, updated_at: new Date().toISOString() });
    if (result.error) return toast.error("Falha ao excluir conta.");
